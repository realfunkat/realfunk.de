...
def update_magazine(arts):
 ...
 if len(items)<4:raise ValueError('Mindestens vier vollständige Hero-Einträge erforderlich')
 ...
 if 'src="hero-rotation.js' not in s:s=s.replace('</body>','<script defer src="hero-rotation.js?v=20260916-kinderschutz"></script></body>')
 s=re.sub(r'hero-rotation\.js\?v=[^"\']+', 'hero-rotation.js?v=20260916-kinderschutz', s)
 small='<section class="small-heroes" aria-label="Weitere Top-Themen">'+''.join(f'''<a class="mini-hero" href="artikel/{esc(x['file'])}"><img src="images/{esc(x['img'])}" alt="{esc(x.get('alt',''))}" loading="lazy"><div><span>{esc(x['kick'])}</span><h2>{esc(x['ttl'])}</h2></div></a>''' for x in items[1:4])+'</section>'
 ...
 hero_files={item['file'] for item in items[:4]}
 rows=[]
 for topic in active:

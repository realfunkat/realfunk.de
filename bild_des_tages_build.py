...
def detail_page(item: dict[str, str], older: dict[str, str] | None, newer: dict[str, str] | None) -> str:
    ...
    show_comment = item.get("show_comment", True)
    ...
    return f'''<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(item["title"])} — Bild des Tages — RealFunk</title><link rel="stylesheet" href="../magazin.css"><link rel="stylesheet" href="../bild-des-tages.css?v=20260913"><link rel="stylesheet" href="../realfunk-shell.css?v=20260908"><link rel="canonical" href="{page_url}">{rel_links}<meta name="description" content="{escape(description, quote=True)}"><meta property="og:type" content="article"><meta property="og:title" content="{escape(item["title"], quote=True)} — Bild des Tages"><meta property="og:description" content="{escape(description, quote=True)}"><meta property="og:url" content="{share_url}"><meta property="og:image" content="{og_image}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="{escape(item["title"], quote=True)} — Bild des Tages"><meta name="twitter:description" content="{escape(description, quote=True)}"><meta name="twitter:image" content="{og_image}"><link rel="stylesheet" href="../realfunk-share.css?v=1"></head><body class="subpage">{header("../")}<main class="{main_class}"><div class="daily-heading"><a href="../bild-des-tages.html">← Alle Bilder des Tages</a><span class="eyebrow">BILD DES TAGES · {date_de(item["date"])}</span><h1>{escape(item["title"])}</h1>{share_block(item["title"], share_url)}</div><figure><div class="daily-image-frame"><img src="../images/{escape(item["image"], quote=True)}" alt="{escape(item["alt"], quote=True)}"></div>{caption}</figure><nav class="daily-sequence" aria-label="Zwischen den Bildern des Tages wechseln">{nav_item(older, "older") }<a class="daily-nav-all" href="../bild-des-tages.html">Alle Bilder</a>{nav_item(newer, "newer")}</nav></main>{footer("../")}<script defer src="../realfunk-share.js?v=2"></script></body></html>'''


def archive_page(entries: list[dict[str, str]]) -> str:
    latest = entries[0]
    cards = "".join(
        f'''<article class="daily-gallery-card"><a href="bild-des-tages/{escape(filename(item), quote=True)}"><div class="daily-gallery-image"><img src="images/{escape(item["image"], quote=True)}" alt="{escape(item["alt"], quote=True)}" loading="lazy"></div><div class="daily-gallery-copy"><time datetime="{item["date"]}">{date_de(item["date"])}</time><h2>{escape(item["title"])}</h2>{f'<p>{escape(item["teaser"])}</p>' if item["teaser"] else ''}<span>Ansehen →</span></div></a></article>'''
        for item in entries
    )
...
def update_homepage(latest: dict[str, str]) -> None:
    ...
    teaser = f'<p>{escape(latest["teaser"])}</p>' if latest["teaser"] else ""
    replacement = f'''<a class="format-tile daily-picture-tile" href="bild-des-tages/{escape(filename(latest), quote=True)}" target="_blank" rel="noopener"><img src="images/{escape(latest["image"], quote=True)}" alt="{escape(latest["alt"], quote=True)}" loading="lazy"><div class="tile-copy"><span class="eyebrow">Bild des Tages</span><h3>{escape(homepage_title)}</h3>{teaser}<span class="tile-cta">Bild ansehen →</span></div></a>'''

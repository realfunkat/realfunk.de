...
const stage=$('fritz-stage'),spinVideo=$('fritz-spin'),resultVideo=$('fritz-result');
...
function showVideo(kind){
 ...
 const videos=[...incoming.querySelectorAll('video')];for(const v of videos){v.currentTime=0;v.muted=true}
 Promise.all(videos.map(v=>v.play())).then(reveal).catch(()=>{if(generation===videoGeneration)showVideo('idle')});
}
resultVideo.addEventListener('ended',()=>{if(stage.dataset.state==='result')showVideo('idle')});
for(const v of stage.querySelectorAll('video'))v.addEventListener('error',()=>{if(stage.dataset.state!=='idle')showVideo('idle')});
...
function spin(){if(busy)return;busy=true;if(!bag.length)refill();current=bag.pop();const x=lexicon[current],duration=reduced?220:4042;draw.disabled=true;lever.setAttribute('aria-disabled','true');machine.classList.remove('landed');machine.classList.add('spinning');$('pair').setAttribute('aria-busy','true');$('status').textContent='DIE FLOSKEL DREHT SICH …';lever.style.setProperty('--pull','105px');showVideo('spin');startSound(duration/1000);setTimeout(()=>lever.style.setProperty('--pull','0px'),300);setTimeout(()=>{
$('quote').textContent='„'+x.phrase+'“';$('translation').textContent=x.translation;$('speaker').textContent=x.speaker;$('position').textContent=(lexicon.length-bag.length)+' / '+lexicon.length;$('context').textContent=x.context;$('source').href=x.source;machine.classList.remove('spinning');machine.classList.add('landed');$('pair').setAttribute('aria-busy','false');$('status').textContent='KLARTEXT. BITTE SEHR.';showVideo('result');draw.disabled=false;lever.removeAttribute('aria-disabled');busy=false;
},duration)}
